...
async def lifespan(app: FastAPI):
    ...
    with Session(engine) as session:
        if not session.exec(select(Campaign)).first():
            session.add_all([
                Campaign(name="prevent mob lynching" , due_date=datetime.now()),
                Campaign(name="black life matters" , due_date=datetime.now())
...
async def read(session: SessionDep):
    data = session.exec(select(Campaign)).all
    return {"data": data}
 

@app.get("/campaigns/{id}", response_model=Response[Campaign])
async def readCampaigns (id : int, session: SessionDep):
